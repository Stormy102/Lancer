...
class BaseModule(object):
    ...
    @classmethod
    def execute(cls, ip: str, port: int) -> None:
    ...
    def create_loot_space(self, ip: str, port: int):
        ...
        if port not in Loot.loot[ip]:
            Loot.loot[ip][port] = {}
        if self.loot_name not in Loot.loot[ip][port]:
            Loot.loot[ip][port][self.loot_name] = {}

    @classmethod
    def should_execute(cls, service: str, port: int) -> bool:
        return True

    @classmethod
    def can_execute_module(cls) -> ModuleExecuteState:
        """
        Checks if the current module can be executed
        :return: ModuleExecuteState
        """
        # Loop through every program required
        for program in cls.required_programs:
